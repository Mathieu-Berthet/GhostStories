...
public class GreenPlayer : MonoBehaviour
{
    ...
    public bool chooseRelance;
    public bool alreadyRelaunch;
    ...
    public string colorPlayer = "green";
    ...
    public Transform greenPosStall;
    public Transform greenPosHouse;
    public Transform greenPosHut;
    public Transform greenPosPavillon;
    public Transform greenPosGraveyard;
    public Transform greenPosAutel;
    public Transform greenPosCircle;
    public Transform greenPosTemple;
    public Transform greenPosTower;
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.B) && greenTurn)
        {
            CheckDistance();
        ...
        else if (gm.state == GameManager.STATE_GAME.STATE_MOVE && !stop && greenTurn)
        {
            stop = true;
            CheckDistance();
            StartCoroutine(gameObject.GetComponent<Deplacement>().PlayerDeplacement());
    ...
    public void DrawAGhost()
    {
        ...
        if ((gm.state == GameManager.STATE_GAME.STATE_DRAW || useTilePower || useGhostPower) && greenTurn)
        {
            ...
            if (gm.greenBoard.nbCardOnBoard == 3 && !useTilePower)
            {
                textInfo.gameObject.SetActive(true);
    ...
    private void updateUI()
    {
        ...
        textNbTokenYinYangGreen.text = "x " + NbYinYangGreenToken;
    ...
    {
        if (greenTurn && !powerForceDeLaMontagne)
        {
            chooseRelance = false;
            relance = " ";
            canLaunchBlackDice = false;
    ...
    public void ActivateInGameEffect()
    {
        if (greenTurn)
        {
            ...
            for (int i = 0; i < maxChild; i++)
            {
                ...
                {
                    if (gm.greenBoard.gameObject.transform.GetChild(i).GetChild(4).GetComponent<Ghost>().inGamePower && !gm.greenBoard.gameObject.transform.GetChild(i).GetChild(4).name.Contains("Bouddha"))
                    {
                        gm.greenBoard.gameObject.transform.GetChild(i).GetChild(4).GetComponent<Ghost>().UseInGamePower(gameObject);
                    }
                }
            }
        }
    ...
    {
        if (greenTurn || useRedPower || useWindCelestialPower)
        {
            switch (tileName)
            {
                case "MaisonThe":
    ...
    public void ChooseToRelaunch(Button buttonClick)
    {
        if (greenTurn)
        {
            relance = buttonClick.transform.GetChild(0).GetComponent<Text>().text;
            chooseRelance = true;
        }
    }
